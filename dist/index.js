...
const UploadDropzone = (props) => {
  const { children } = props, {
    ...
    if (Array.isArray(event))
      return [];
    if ("type" in event && event.type === "drop" && "dataTransfer" in event && (fileList = event?.dataTransfer?.files), "type" in event && event.type === "change") {
    ...
    accept: isImageAssetType ? { "image/*": [] } : void 0,
